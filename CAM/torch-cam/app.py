...
def main():
    ...
    tv_model = st.sidebar.selectbox("Classification model", TV_MODELS, help="Supported models from Torchvision")
    ...
    if st.sidebar.button("Compute CAM"):
        if uploaded_file is None:
            ...
            with st.spinner("Analyzing..."):
                ...
                confidence_score = probs.squeeze(0)[class_idx].item()
                ...
                ax.axis("off")
                cols[-1].pyplot(fig)
                cols[-1].markdown(f"#### Score de confiance : {confidence_score:.2%}")
